...
def brush_function():
    ...
    # Resize the front image to the output size
    img_front_resized = cv2.resize(img_front, (output.shape[1], output.shape[0]))
    
    # Copy and paste regions from the front image
    for rect in rectangles:
        x, y = rect
        radius = 20  # Set the desired radius of the circular region
        
        # Copy the corresponding region from the front image
        copied_region = img_front_resized[y - radius:y + radius, x - radius:x + radius]
